							...
							<div class="row no-gutters justify-content-center">
								<div class="col-12 shadow white-bg rounded my-md-3 my-0 p-4 p-sm-5">
									<div class="pb-3">	
										...
										<h3 class="brown m-0">Edit Agenda</h3>	
									</div>
									<form method="POST" action="{{ url('/mentors/update-agenda',array($agenda->id)) }}">
									    @csrf
											...
											<div class="edit-section-body">
												<div class="edit-body-row">
													<div class="pad15">
														...
														<div class="padT10">
															<input name="title" type="text" class="edit-title-input col-12 padLR10" value="{{ $agenda->title }}" required="required">	
															@if ($errors->has('title'))
												...
												<div class="edit-body-row">
													<div class="pad15">
														...
														<div class="padT10 new-profile-form">
															<textarea name="description" rows="3" class="mceDashboard">{{ $agenda->description }}</textarea>	
												...
												<div class="edit-body-row event-details-wrap">
													@if(count($agenda->details)>0)
														@foreach($agenda->details as $key => $detail)
															...
															<span>Event Details</span>
																		...
																		@php
													      				...
													      				@foreach($dates as $date)
																        	...
																        	<div class="session-date-inner mt-2">
																				<select name="sd-day[{{$key}}][]" class="custom-select new-profile-form sd-day">	
																					
																					@for($day=1;$day<=31;$day++)			
																					    <option value='{{ sprintf("%02d",$day) }}' 
																							@if(!empty($date[0]))
																							    {{ $date[0] == $day ? 'selected="selected"' : '' }}
																						    @endif	
																					    >{{ sprintf("%02d",$day) }}</option>
																					@endfor
																				</select>	
																				<span> / </span>	
																				<select name="sd-month[{{$key}}][]" class="custom-select new-profile-form sd-month">
																					
																					@for($month=1;$month<=12;$month++)				
																					    <option value='{{ sprintf("%02d",$month) }}'	
																							@if(!empty($date[1]))
																							    {{ $date[1] == $month ? 'selected="selected"' : '' }}
																						    @endif									    
																					    >{{ sprintf("%02d",$month) }}</option>		
																					@endfor
																				</select>	
																				<span> / </span>	
																				<select name="sd-year[{{$key}}][]" class="custom-select new-profile-form sd-year">
																					
																					@for($year=date("Y");$year>=2020;$year--)	
																					    <option value='{{ $year }}' 							    
																							@if(!empty($date[2]))
																							    {{ $date[2] == $year ? 'selected="selected"' : '' }}
																						    @endif
																					    >{{ $year }}</option>								
																					@endfor
																				</select>
																...
																<div class="session-time-wrap py-2">
																	...
																	<div class="session-time">
																		<div class="time-picker">	
																			<div class="hour">
																				<div class="hr-up"></div>
																				<input type="number" class="hr start-hr" value="{{$startTime[0]}}" name="st-start-hr[{{$key}}]">
																			...
																			<div class="minute">
																				...
																				<input type="number" class="min start-min" value="{{$startTime[1]}}" name="st-start-min[{{$key}}]">
																		...
																		<div class="time-picker">
																			<div class="hour">
																				<div class="hr-up"></div>
																				<input type="number" class="hr end-hr" value="{{$endTime[0]}}" name="st-end-hr[{{$key}}]">
																				<div class="hr-down"></div>
																			</div>
																			...
																			<div class="minute">
																				<div class="min-up"></div>
																				<input type="number" class="min end-min" value="{{$endTime[1]}}" name="st-end-min[{{$key}}]">
																				<div class="min-down"></div>
																			</div>
																		</div>															
																	</div>															
																...
																<div class="row">
																	<div class="col-12 col-md-6 col-lg-5 col-xl-4 session-venue py-2">
																		...
																		<input type="text" name="venue[{{$key}}]" value="{{ $detail->venue }}" class="venue-input">
																	</div>
																	<div class="col-12 col-md-6 col-lg-7 col-xl-8 session-fee py-2">
																		<p class="green">Fee</p>
																		<span >€ </span><input type="number" name="fee[{{$key}}]" class="number-input fee-input" min="1" max="1000" step=".10" placeholder="0.00" value="{{ $detail->fee }}">
																	</div>	
																	<div class="col-12 col-md-6 col-lg-5 col-xl-4 session-language py-2">
																		<p class="green">Language</p>
																		<select name="language[{{$key}}]" class="custom-select new-profile-form lang-select">
																			@foreach($languages as $language)
																				<option value="{{ $language }}"
																				@if(!empty($detail->language))
																				    {{ $detail->language == $language ? 'selected="selected"' : '' }}
																			    @endif
																				>{{ $language }}</option>
																			@endforeach
																		</select>
																		<div class="other mt-2">
																			...
																			<input type="text" name="other-lang[{{$key}}]" value="{{ $detail->other_language }}" class="lang-other">
																		</div>	
																	</div>
																	<div class="col-12 col-md-6 col-lg-7 col-xl-8 session-promo py-2">
																		<p class="green">Promo</p>
																		<input type="text" name="promo[{{$key}}]" value="{{ $detail->promo }}" class="promo-input">
																	</div>	
																</div>
														</div>
														@endforeach
														...
														<div class="event-details pad15 js-evt-count">
															<span>Event Details</span>
																...
																<div class="row">
																	...
																	<div class="col-12 col-md-6 col-lg-7 col-xl-8 session-language py-2">
																		<p class="green">Language</p>
																	...
																	<div class="col-12 col-md-6 col-lg-5 col-xl-4 session-fee py-2">
																		<p class="green">Fee</p>
...
<div class="blank-event-dtls hide">
	<div class="event-details pad15 js-evt-count">
		<span>Event Details<span class="required"> *</span></span>
		<button type="button" class="brown del-evt-dtls-btn new-profile-form pull-right"><i class="fas fa-trash"></i></button>
		<div class="session-date-wrap py-2">
			<p class="green">Session Date</p>
			<div class="session-date inlineBlock">
