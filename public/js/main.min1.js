...
jQuery( document ).ready(function( $ ) {
  ...
  // nanny add reference
  $('.add-ref-btn').click(function() {
    $('.refs-item-wrap:last').clone().appendTo('.refs-parent').find("input").val("");  
    $('.refs-item-wrap').each(function() {
      if (!$(this).children('.del-ref-item').length) {
        $(this).append('<button type="button" class="brown del-ref-item new-profile-form"><i class="fas fa-trash"></i></button>');
      }      
    });
  });

  $('.refs-item-wrap').each(function() {
    if ($('.refs-item-wrap').length > 1) {
      if (!$(this).children('.del-ref-item').length) {
        $(this).append('<button type="button" class="brown del-ref-item new-profile-form hide"><i class="fas fa-trash"></i></button>');
      }   
    }
  });

  // nanny delete reference input
  $('.refs-parent').on('click','.del-ref-item', function(){
    if ($('.refs-item-wrap').length == 2) {
      $(this).parent('.refs-item-wrap').remove();
      $('.del-ref-item').remove();
       
    } else if ($('.refs-item-wrap').length > 2) {
      $(this).parent('.refs-item-wrap').remove();
    }
  });
  ...
  // badge screening modal
  $('#badgeScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    $(e.currentTarget).find('input[name="userId"]').val(id);
  });

  // remarks screening modal
  $('#remarksScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    var remarks = $(e.relatedTarget).data('remarks');
    $(e.currentTarget).find('input[name="userId"]').val(id);
    $(e.currentTarget).find('textarea[name="remarks"]').val(remarks);
  ...
  // process screening modal
  $('#processScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    var name = $(e.relatedTarget).data('name');
    $(e.currentTarget).find('input[name="userId"]').val(id);
    $(e.currentTarget).find('.user-name').html(name);
  ...
  // pend screening modal
  $('#pendScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    var name = $(e.relatedTarget).data('name');
    $(e.currentTarget).find('input[name="userId"]').val(id);
    $(e.currentTarget).find('.user-name').html(name);
  ...
  // pass screening modal
  $('#passScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    var name = $(e.relatedTarget).data('name');
    $(e.currentTarget).find('input[name="userId"]').val(id);
    $(e.currentTarget).find('.user-name').html(name);
  ...
  // fail screening modal
  $('#failScrModal').on('show.bs.modal', function(e) {
    var id = $(e.relatedTarget).data('id');
    var name = $(e.relatedTarget).data('name');
    $(e.currentTarget).find('input[name="userId"]').val(id);
    $(e.currentTarget).find('.user-name').html(name);
  ...
  // SORTH TABLE
  $('.table.dashboard-table th').click(function(){
  ...
  $('.table.table-vetting th').click(function(){
    var table = $(this).parents('table').eq(0);
  ...
  const $menu = $('.ut-dropdown,.ad-dropdown,.vs-dropdown');
